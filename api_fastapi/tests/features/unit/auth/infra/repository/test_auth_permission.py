...
from app.features.auth.infra.repository.permission_repository import PermissionRepository
...
class TestAuthPermissionRepository:
    ...
    async def test_list_permissions(self):
        ...
        assert result == [
            Permission(id=1, name="manage_users", description="Can manage users", roles=[]),
            Permission(id=2, name="view_reports", description="Can view reports", roles=[]),
        ]
